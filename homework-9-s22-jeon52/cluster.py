...
class Cluster:
    ...
    def avgDistance(self):
        ...
        for i in self.points: # i -> elements in the list
            sum += i.distFrom(self)
        
        averagedistance = sum / len(self.points)
    ...
    def updateCenter(self):
        ...
        if (len(self.points) == 0):
            ...
            for i in range(self.center.dim):
                updated = [p[i] for p in self.points]
                self.center.coords[i] = sum(updated) / len(self.points)
